...
import type { EnFields } from './en'
import type { ZhFields } from './zh'
...
type InferDocumentType<Q> =
  Q extends { collection: 'en' } ? OutstaticSchema<EnFields> :
  Q extends { collection: 'zh' } ? OutstaticSchema<ZhFields> :
  OutstaticSchema<AllDocuments>

/** API returned by load() */
interface LoadAPI {
  /** Find en documents with field projection (use 'as const' for type narrowing) */
  find<K extends keyof OutstaticSchema<EnFields>>(
    query: { collection: 'en' } & Record<string, unknown>,
    projection: readonly K[]
  ): FindAPI<OutstaticSchema<EnFields>, Pick<OutstaticSchema<EnFields>, K>>

  /** Find en documents */
  find(
    query: { collection: 'en' } & Record<string, unknown>,
    projection: string[]
  ): FindAPI<OutstaticSchema<EnFields>>
  ...
  /** Find zh documents with field projection (use 'as const' for type narrowing) */
  find<K extends keyof OutstaticSchema<ZhFields>>(
    query: { collection: 'zh' } & Record<string, unknown>,
    projection: readonly K[]
  ): FindAPI<OutstaticSchema<ZhFields>, Pick<OutstaticSchema<ZhFields>, K>>

  /** Find zh documents */
  find(
    query: { collection: 'zh' } & Record<string, unknown>,
    projection: string[]
  ): FindAPI<OutstaticSchema<ZhFields>>
